...
<head>
    ...
    <style>
        ...
        button { width: 100%; padding: 12px; background-color: #3b82f6; color: white; border: none; border-radius: 5px; font-size: 16px; font-weight: bold; cursor: pointer; margin-top: 10px; }
    ...
    <form action="guardar_compra.php" method="POST">
        ...
        <div class="form-group">
            ...
            <select name="proveedor_id" required>
                ...
                <?php
                // Se utiliza 'proveedor_id' (o 'id' según la estructura de tu tabla proveedores)
                $res_prov = $conn->query("SELECT proveedor_id, nombre_empresa FROM proveedores ORDER BY nombre_empresa ASC");
                if (!$res_prov) {
                    // Respaldo por si la tabla proveedores mantiene el nombre 'id'
                    $res_prov = $conn->query("SELECT id AS proveedor_id, nombre_empresa FROM proveedores ORDER BY nombre_empresa ASC");
                }
                while($prov = $res_prov->fetch_assoc()) {
                    echo "<option value='" . $prov['proveedor_id'] . "'>" . htmlspecialchars($prov['nombre_empresa']) . "</option>";
                }
                ?>
            </select>
        </div>
        ...
        <div class="form-group">
            ...
            <select name="producto_id" required>
                ...
                <?php
                // CORRECCIÓN: Se cambia 'id' por 'producto_id'
                $res_prod = $conn->query("SELECT producto_id, nombre_producto FROM productos ORDER BY nombre_producto ASC");
                while($prod = $res_prod->fetch_assoc()) {
                    echo "<option value='" . $prod['producto_id'] . "'>" . htmlspecialchars($prod['nombre_producto']) . "</option>";
                }
                ?>
            </select>
        </div>
